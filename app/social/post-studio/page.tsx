...
export default function SocialPostStudioPage() {
  ...
  const [humanizerLevel, setHumanizerLevel] =
    useState<HumanizerLevel>("medium");
  const [manualBusy, setManualBusy] = useState<
    "save" | "schedule" | "post_now" | null
  >(null);
  ...
  const onGenerate = async () => {
    ...
    const angles = dedupeBrief([...selectedAngles, ...customAngles]);
    const audiences = dedupeBrief([...selectedAudiences, ...customAudiences]);
    try {
        ...
        run.formatIntelligence
          ? {
              ...
              formatConfidenceScore:
                run.formatIntelligence.formatConfidenceScore,
              reasoning: run.formatIntelligence.reasoning,
              recommendations: run.formatIntelligence.recommendations.map(
                (r) => ({
                  label: r.label,
                  score: r.score,
                  reason: r.reason,
  ...
  const requireLinkedInToPublish = () => {
    if (linkedinConnected) return true;
    toast.error("Connect LinkedIn to schedule or publish posts.");
    return false;
  };

  const approveLatest = async (postNow = false) => {
    if (!latestRun) return;
    if (!requireLinkedInToPublish()) return;
    const lengthError = getLinkedinPostLengthError(
      latestRun.draft.body.trim().length,
    );
    if (lengthError) {
  ...
  const manualMergedHashtags = useMemo(() => {
    ...
    const fromBody = Array.from(manualBody.matchAll(/(^|\s)#(\w+)/g)).map(
      (match) => match[2],
    );
  ...
  const onManualSchedule = async () => {
    ...
    try {
      ...
      const response = await schedulePost(
        id,
        new Date(manualScheduleFor).toISOString(),
      );
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description={
          linkedinConnected
            ? "Use AI Post Builder or write your own post in a LinkedIn-style studio and schedule directly."
            : "Generate and preview posts without LinkedIn. Connect your account when you are ready to schedule or publish."
        }
        actions={
      ...
      {!linkedinConnected && (
        <div className="mb-5">
          <InlineAlert
            tone="info"
            title="Preview mode"
            description="You can generate AI drafts and save manual drafts. Connect LinkedIn to schedule or publish to your profile."
          />
        </div>
      )}
        ...
        {flow === "ai" ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <p className="mb-2 text-xs text-slate-500">
                  Select one or more angles. Custom entries can be
                  comma-separated.
                </p>
                <div className="flex flex-wrap gap-2">
              ...
              <div>
                ...
                <p className="mb-2 text-xs text-slate-500">
                  Select one or more audiences. Custom entries can be
                  comma-separated.
                </p>
                <div className="flex flex-wrap gap-2">
                ...
                {showCustomAudience && (
                  <input
                    ...
                    onChange={(event) =>
                      setCustomAudienceText(event.target.value)
                    }
                    ...
                    <>
                      ...
                      {hasValidByok && form.createImage && (
                        <p className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
                          Your BYOK provider is active. All image generation
                          costs are charged directly by your provider, giving
                          you full control over usage and billing.
                        </p>
                      )}
                    </>
                  )}
          ...
          <SurfaceCard className="lg:col-span-5">
            <SectionTitle
              ...
              description={
                linkedinConnected
                  ? "Write your own post, preview it in real-time, then save, schedule, or post now."
                  : "Write and preview your post, then save a draft. Connect LinkedIn to schedule or publish."
              }
            />
            <LinkedinStudioEditor
              publishEnabled={linkedinConnected}
              body={manualBody}
        ...
        <div className="border-r border-slate-200">
                  ...
                  {mediaUrls.map((url) => (
                    <div key={url} className="relative">
                      ...
                      <button
                        ...
                        onClick={() =>
                          onMediaUrlsChange(
                            mediaUrls.filter((item) => item !== url),
                          )
                        }
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <input
              ...
              className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs text-slate-700 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-50"
            />
            <PrimaryButton
              onClick={onSchedule}
              disabled={
            ...
            {!publishEnabled && (
              <span className="text-[11px] text-slate-500">
                Connect LinkedIn to schedule or publish.
              </span>
            )}
          </div>
        </div>
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
        ...
        {run.formatIntelligence ? (
          <div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3">
            ...
            <p className="mt-1 text-xs text-violet-900">
              ...
              {Math.round(run.formatIntelligence.formatConfidenceScore * 100)}%
              confidence
            </p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {approvalDone && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
              This draft is already approved. Check the approval queue or
              calendar for the scheduled publish time.
            </div>
          )}
          {!terminal && (
            <>
              <PrimaryButton
                ...
                disabled={!publishEnabled || busy || draftOverLimit}
              >
                <IconBrandLinkedin className="h-4 w-4" />
                Approve · schedule
              </PrimaryButton>
              ...
              {!publishEnabled && (
                <p className="w-full text-xs text-slate-500">
                  Preview only — connect LinkedIn to approve, schedule, or publish.
                </p>
              )}
            </>
          )}
      ...
      <SurfaceCard className="lg:col-span-2">
        ...
        <dl className="space-y-3 text-sm">
          <MetaRowItem
            label="Agent run id"
            value={run.agentRunId.slice(0, 8)}
          />
