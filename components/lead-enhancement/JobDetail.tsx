...
import {
    downloadJobFile, getJob, getJobRows, LeadEnhancementJob, pushJobToSheet
} from '@/utils/api/leadEnhancementClient';
import {
    IconCheck, IconClock, IconDownload, IconFileSpreadsheet, IconX
...
export default function JobDetail({ jobId }: { jobId: string }) {
  ...
  const handlePushToSheet = async () => {
    const url = window.prompt(
      "Paste public Google Sheet link to write results"
    );
    if (!url) return;

    try {
      toast.loading("Pushing to sheet...", { id: "push-sheet" });
      await pushJobToSheet(jobId, url);
      toast.success(
        "Results pushed to sheet successfully (if webhook configured)",
        { id: "push-sheet" }
      );
    } catch (e: any) {
      console.error("Push to sheet failed:", e);
      toast.error(e?.message || "Failed to push to sheet", {
        id: "push-sheet",
      });
    }
  };
            ...
            <div className="p-6">
              <div className="flex gap-3 justify-end">
                ...
                <Button
                  ...
                  disabled={job?.status !== "completed"}
                  className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white px-4 py-2 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                >
                  <div className="flex items-center gap-2">
                    <svg
                      className="h-4 w-4"
